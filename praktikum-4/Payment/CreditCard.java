class CreditCard implements IPaymentMethod {
    ...
    public boolean pay(double price) {
        if(this.monthlyBalance + price > this.monthlyLimit){return false;}
        this.monthlyBalance = this.monthlyBalance + price;
        return true;
    }
    // TODO
    ...
    public boolean payOffBalance(double amount) {
        if(amount > this.monthlyBalance + this.unpaidBalance){return false;}
        if(amount > this.monthlyBalance){
            amount = amount - this.monthlyBalance;
            this.monthlyBalance = 0;
            this.unpaidBalance = this.unpaidBalance - amount;
        } else {
            this.monthlyBalance = this.monthlyBalance - amount;
            amount = 0;
        }
        return true;
    }
